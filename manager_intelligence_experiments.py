...
def _segment_metrics(
    ...
    if int(mask.sum()) == 0:
        return {
            ...
            "draw_recall": np.nan,
            "draw_log_loss": np.nan,
        }
    segment_y = y_true[mask]
    segment_probabilities = probabilities[mask]
    segment_predictions = predictions[mask]
    metrics = evaluate_probabilities(segment_y, segment_probabilities, segment_predictions)
    calibration = calibration_table(pd.Series(segment_y), segment_probabilities)
    draw_actual = (segment_y == 1).astype(int)
    draw_prob = np.clip(segment_probabilities[:, 1], 1e-12, 1 - 1e-12)
    draw_predictions = (segment_predictions == 1).astype(int)
    return {
        "matches": float(mask.sum()),
        "accuracy": metrics["accuracy"],
        "log_loss": metrics["log_loss"],
        "Brier_score": metrics["brier_score_multiclass"],
        "expected_calibration_error": expected_calibration_error(calibration),
...
def manager_change_segment_metrics(results: list[dict[str, object]], dataset: pd.DataFrame) -> pd.DataFrame:
    manager_signal = dataset[
        [
            "home_new_manager_first_5_matches",
            "away_new_manager_first_5_matches",
            "home_new_manager_first_10_matches",
            "away_new_manager_first_10_matches",
        ]
    ]
    rows: list[dict[str, object]] = []
    for result in results:
        split = result["split"]
        signal_test = manager_signal.loc[split.X_test.index].reset_index(drop=True)
        y_true = split.y_test.to_numpy()
        probabilities = result["probabilities"]
        predictions = result["predictions"]
        segments = {
            "all_test_matches": np.ones(len(signal_test), dtype=bool),
            "home_new_manager_first_5": signal_test["home_new_manager_first_5_matches"].to_numpy() > 0,
            "away_new_manager_first_5": signal_test["away_new_manager_first_5_matches"].to_numpy() > 0,
            "any_new_manager_first_5": (
                signal_test["home_new_manager_first_5_matches"].to_numpy()
                + signal_test["away_new_manager_first_5_matches"].to_numpy()
            )
            > 0,
            "home_new_manager_first_10": signal_test["home_new_manager_first_10_matches"].to_numpy() > 0,
            "away_new_manager_first_10": signal_test["away_new_manager_first_10_matches"].to_numpy() > 0,
            "any_new_manager_first_10": (
                signal_test["home_new_manager_first_10_matches"].to_numpy()
                + signal_test["away_new_manager_first_10_matches"].to_numpy()
            )
            > 0,
            "no_new_manager_first_10": (
                signal_test["home_new_manager_first_10_matches"].to_numpy()
                + signal_test["away_new_manager_first_10_matches"].to_numpy()
            )
            == 0,
        ...
        for segment, mask in segments.items():
            row = {"model_version": result["model_version"], "segment": segment}
            row.update(_segment_metrics(y_true, probabilities, predictions, mask))
            rows.append(row)
    output = pd.DataFrame(rows)
    output.to_csv(OUTPUT_DIR / "manager_change_segment_metrics.csv", index=False)
    return output
...
"""
    ...
    segment_path = OUTPUT_DIR / "manager_change_segment_metrics.csv"
    segment_text = "Segment metrics were not generated."
    if segment_path.exists():
        segment_metrics = pd.read_csv(segment_path)
        segment_text = _markdown_table(
            segment_metrics[
                segment_metrics["segment"].isin(
                    ["all_test_matches", "any_new_manager_first_5", "any_new_manager_first_10", "no_new_manager_first_10"]
                )
            ],
...
def main() -> None:
    ...
    segment_metrics = manager_change_segment_metrics(results, dataset)
        ...
        json.dumps(
            {
                ...
                "new_manager_first_10_test_matches": int(
                    segment_metrics[
                        (segment_metrics["model_version"] == "model_d_full_manager_intelligence")
                        & (segment_metrics["segment"] == "any_new_manager_first_10")
                    ]["matches"].iloc[0]
                ),
                "activate": bool(
